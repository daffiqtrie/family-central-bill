...
class BillOrchestrator:
    ...
    async def get_bill_data(
        ...
        if not force_refresh:
            ...
            if cached_data and self._is_cache_fresh(cached_data):
                logger.debug("Bill cache hit for provider=%s", provider_str)
        ...
        try:
            logger.debug(
                "Bill cache miss for provider=%s; calling checker", provider_str
            )
            ...
            if response.status in (
                ...
                logger.debug("Bill cache updated for provider=%s", provider_str)

            return response

        except Exception:
            logger.exception("Bill checker failed for provider=%s", provider_str)

            # Step 4: Fallback to stale cache if available
            if cached_data:
                logger.info(
                    "Falling back to stale bill cache for provider=%s", provider_str
                )
                response = self._cache_to_response(cached_data)
                response.message = (
                    f"[STALE CACHE] {response.message or 'Data dari cache'}"
                )
                return response
    ...
    def _cache_to_response(self, cache: BillCache) -> BillCheckResponse:
        ...
        return BillCheckResponse(
            ...
            total_amount=Decimal(cache.amount or 0),
            checked_at=cache.updated_at,
