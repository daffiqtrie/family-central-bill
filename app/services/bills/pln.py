...
def get_stealth_headers() -> dict[str, str]:
    ...
    """
    # Get random User-Agent or fallback to static one
    if _ua:
    ...
    return {
        ...
        # Required authentication headers for Sepulsa API
...
class SepulsaResponseParser:
        ...
        try:
            ...
            if settings.DEBUG:
                ...
                if isinstance(data, dict):
                    ...
                    print(f"[PLN DEBUG] first item keys: {data[0].keys() if isinstance(data[0], dict) else 'not dict'}")
            ...
            if settings.DEBUG and lines:
                print(f"[PLN DEBUG] lines[0] keys: {lines[0].keys() if isinstance(lines[0], dict) else 'not dict'}")
            
            attributes = lines[0].get("attributes", []) if isinstance(lines[0], dict) else []
            ...
            for attr in attributes:
                ...
                if attr_code == "inquiry_details":
                    ...
                    if settings.DEBUG:
                        print(f"[PLN DEBUG] ✅ Found inquiry_details! Type: {type(details_str)}")
                        if isinstance(details_str, str):
                            print(f"[PLN DEBUG] inquiry_details content: {details_str[:300]}")
    ...
    def parse_response(
        ...
        tarif_daya = (
            details.get("Tarif / Daya") or  # With spaces (actual Sepulsa format)
            details.get("Tarif/Daya") or    # Without spaces
            details.get("tarif_daya", "")
        )
        ...
        jumlah_tagihan = cls.parse_currency(
            details.get("Jumlah Tagihan") or 
            details.get("jumlah_tagihan") or "0"
        )
        
        # Admin fee - Sepulsa uses "Biaya Admin", not "Admin Bank"
        admin_fee = cls.parse_currency(
            details.get("Biaya Admin") or
            details.get("Admin Bank") or 
            details.get("admin_bank") or "2500"
        )
        
        # Total - calculate if not provided
        total_bayar = cls.parse_currency(
            details.get("Total Bayar") or 
            details.get("total_bayar") or "0"
        )
...
class PLNBillChecker(BaseBillChecker):
    ...
    async def _fetch_from_sepulsa(self, customer_id: str) -> BillCheckResponse:
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                if settings.DEBUG:
                    print(f"[PLN] Session cookies obtained: {len(client.cookies)} cookies")
            except Exception as e:
                if settings.DEBUG:
                    ...
                    if errors:
                        ...
                        if error_code == "50":
                            return BillCheckResponse(
                                ...
                                customer_name="",
                                status=BillStatus.PAID,
                                message=error_detail or "Tagihan sudah lunas / tidak tersedia",
                            )
                        ...
                        if error_code == "20":
                            return BillCheckResponse(
                                ...
                                customer_name="",
                                status=BillStatus.NOT_FOUND,
                                message=error_detail or "Nomor salah / terblokir / expired",
                            )
            ...
            if settings.DEBUG:
                print(f"[PLN] Response data keys: {response_data.keys() if isinstance(response_data, dict) else 'not dict'}")
    ...
    async def _generate_mock_response(self, customer_id: str) -> BillCheckResponse:
        ...
        names = [
            ...
            "RUDI HARTONO", "ANI SUSANTI", "DJALEKA", "MAYA SARI",
        ]
