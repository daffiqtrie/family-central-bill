...
SEPULSA_PDAM_PRODUCT_URL = "http://api.sepulsa.com/api/v1/oscar/products/988/"  # PDAM Kota Padang
...
class PDAMResponseParser:
    ...
    def parse_response(
        ...
        jumlah_tagihan = cls.parse_currency(
            details.get("Jumlah Tagihan") or "0"
        )
...
class PDAMBillChecker(BaseBillChecker):
    ...
    async def _fetch_from_sepulsa(self, customer_id: str) -> BillCheckResponse:
        ...
        async with httpx.AsyncClient(
            ...
            if settings.DEBUG:
                print(f"[PDAM] Response data keys: {response_data.keys() if isinstance(response_data, dict) else 'not dict'}")
    ...
    async def _generate_mock_response(self, customer_id: str) -> BillCheckResponse:
        ...
        names = [
            "ROCHMANA JAMIN", "AHMAD RIZKI", "FATIMAH ZAHRA", "YUSUF HAKIM",
            "RAHMAN SYAH", "AISYAH BELLA", "FADHIL AKBAR", "ZAHRA AMELIA",
        ]
