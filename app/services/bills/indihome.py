...
SEPULSA_INDIHOME_PRODUCT_URL = "http://api.sepulsa.com/api/v1/oscar/products/34/"  # Indihome
SEPULSA_OPTION_URL = "https://api.sepulsa.com/api/v1/oscar/options/1/"

# Import stealth headers from PLN module (shared utility)
...
class IndihomeResponseParser:
    ...
    def parse_error_response(
        ...
        for error in errors:
            ...
            if code == "50" or "paid" in detail.lower() or "not available" in detail.lower():
    ...
    def parse_response(
        ...
        jumlah_tagihan = cls.parse_currency(
            ...
            details.get("Total Tagihan") or "0"
        )
        admin_fee = cls.parse_currency(
            details.get("Biaya Admin") or "3000"
        )
...
class IndihomeBillChecker(BaseBillChecker):
        ...
        async with httpx.AsyncClient(
            ...
            if response.status_code == 400:
                try:
                    ...
                    return IndihomeResponseParser.parse_error_response(error_data, customer_id) or BillCheckResponse(
                        provider=ProviderEnum.INDIHOME,
                        customer_id=customer_id,
                        customer_name="",
                        status=BillStatus.NOT_FOUND,
            ...
            if settings.DEBUG:
                print(f"[INDIHOME] Response data keys: {response_data.keys() if isinstance(response_data, dict) else 'not dict'}")
    ...
    async def _generate_mock_response(self, customer_id: str) -> BillCheckResponse:
        ...
        names = [
            "HENDRO WIBOWO", "SANTI PERMATA", "DIMAS ADITYA", "LAURA VALENTINE",
            "KEVIN ANDERSON", "MICHELLE TANAKA", "RYAN HIDAYAT", "DIANA KUSUMA",
        ]
